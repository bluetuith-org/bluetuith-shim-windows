...
{
    public static readonly IStack SelectedStack = AvailableStacks.CurrentStack;
    ...
    public class Server
    {
        ...
        public class Start
        {
            [CliOption(
                Required = false,
                Description = "The full path and the socket name to be created and listened on. For example: 'C:\\Users\\user\\AppData\\Local\\Temp\\socket.sock'"
            )]
            public string SocketPath { get; set; } =
                Path.Combine(Path.GetTempPath(), "bh-shim.sock");
            ...
            public int Run(CliContext context)
            {
                ...
                var error = Systray.Server.Launch(
                    SocketPath,
                ...
                return OperationManager.Offload(
                    ...
                    int (token) =>
                    {
                        ...
                        return Output.Result(
                            features.ToResult("Features", "features"),
                            Errors.ErrorNone,
                            token
                        );
                    }
    ...
    public class Device
    {
        ...
        public class Opp
        {
            ...
            {
                [CliOption(HelpName = "file", Description = "A full path of the file to send.")]
            ...
            public class SendFiles : AddressParameter
            {
                [CliOption(
                    HelpName = "file",
                    Description = "A full path of the file to send. Use '-f' multiple times to send multiple files."
                )]
                ...
                public async Task<int> RunAsync(CliContext context)
                {
                    ...
                    return await OperationManager.OffloadAsync(
                        ...
                        async (token) =>
                        {
                            ...
                            {
                                error = await SelectedStack.SendFileAsync(file);
                                if (error != Errors.ErrorNone)
                                    break;
                            }

                            SelectedStack.StopFileTransferSession();
